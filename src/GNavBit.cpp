//----------------------------------------------------------------------
// GNavBit_Fixed.cpp:
//   Fixed implementation of navigation bit synthesis class for GNAV
...
// Based on ICD GLONASS standard
// Each row represents which data bits contribute to each parity bit
const unsigned char GNavBit::HammingGenerator[8][11] = {
    // Parity bit 1 (position 86) - covers positions 1,3,5,7,...,85 (odd positions)
    {0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0x2A},
    // Parity bit 2 (position 87) - covers positions 2,3,6,7,10,11,...
    {0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0x0C},
    // Parity bit 3 (position 88) - covers positions 4,5,6,7,12,13,14,15,...
    {0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0x10},
    // Parity bit 4 (position 89) - covers positions 8,9,10,11,12,13,14,15,...
    {0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x1F},
    // Parity bit 5 (position 90) - covers positions 16-31,...
    {0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x1F},
    // Parity bit 6 (position 91) - covers positions 32-63,...
    {0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x1F},
    // Parity bit 7 (position 92) - covers positions 64-85
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x3F},
    // Parity bit 8 (position 93) - overall parity of all 92 bits
...
int GNavBit::GetFrameData(GNSS_TIME StartTime, int svid, int Param, int *NavBits)
{
    ...
    // Prepare 85 information bits
    Stream[0] = (data[0] & 0xfffff);      // bits 1-20
    Stream[1] = data[1];                  // bits 21-52
    Stream[2] = data[2] & 0xffffff00;     // bits 53-84 (upper 32 bits)
    
    // Handle string number for almanac strings
    ...
    // Calculate Hamming (93,85) code - 8 parity bits
    hamming_bits = CalculateHammingCode(Stream);
    
    // Add Hamming code bits to positions 86-93 (stored in lower 8 bits of Stream[2])
    Stream[2] |= hamming_bits;

    // GLONASS FDMA Navigation Message Structure (per ICD GLONASS):
    // - Time mark: transmitted separately at the beginning of each string (not part of 100-bit data)
    // - Data: 85 information bits + 8 Hamming code bits = 93 bits
    // - The remaining 7 bits to make 100 bits are filled with zeros
    
    // Output format for 100-bit string:
    // Bits 1-85: Information bits (directly from Stream)
    // Bits 86-93: Hamming code bits
    // Bits 94-100: Zero padding
    
    int bit_index = 0;
    
    // Copy 85 information bits
    // Bits 1-20 from Stream[0]
    for (i = 19; i >= 0; i--)
        NavBits[bit_index++] = (Stream[0] >> i) & 1;
    
    // Bits 21-52 from Stream[1]
    for (i = 31; i >= 0; i--)
        NavBits[bit_index++] = (Stream[1] >> i) & 1;
    
    // Bits 53-84 from Stream[2] (upper 32 bits)
    for (i = 31; i >= 0; i--)
        NavBits[bit_index++] = (Stream[2] >> (i + 8)) & 1;
    
    // Bit 85 from Stream[2] bit 7
    NavBits[bit_index++] = (Stream[2] >> 7) & 1;
    
    // Copy 8 Hamming code bits (86-93)
    for (i = 7; i >= 0; i--)
        NavBits[bit_index++] = (hamming_bits >> i) & 1;
    
    // Fill remaining 7 bits with zeros (94-100)
    for (i = 0; i < 7; i++)
        NavBits[bit_index++] = 0;
...
// Calculate Hamming (93,85) code for GLONASS
// Input: 85 information bits in Stream[0-2]
// Output: 8 parity bits
unsigned char GNavBit::CalculateHammingCode(unsigned int Data[3])
{
    unsigned char parity_bits = 0;
    unsigned char data_bytes[11];
    int i, j;
    
    // Convert 85 bits to byte array for easier manipulation
    // Stream[0] bits 0-19 -> bytes 0-2 (with padding)
    data_bytes[0] = (Data[0] >> 12) & 0xFF;
    data_bytes[1] = (Data[0] >> 4) & 0xFF;
    data_bytes[2] = ((Data[0] & 0xF) << 4) | ((Data[1] >> 28) & 0xF);
    
    // Stream[1] bits 0-31 -> bytes 2-6
    data_bytes[3] = (Data[1] >> 20) & 0xFF;
    data_bytes[4] = (Data[1] >> 12) & 0xFF;
    data_bytes[5] = (Data[1] >> 4) & 0xFF;
    data_bytes[6] = ((Data[1] & 0xF) << 4) | ((Data[2] >> 28) & 0xF);
    
    // Stream[2] bits 8-31 -> bytes 6-9
    data_bytes[7] = (Data[2] >> 20) & 0xFF;
    data_bytes[8] = (Data[2] >> 12) & 0xFF;
    data_bytes[9] = (Data[2] >> 4) & 0xFF;
    data_bytes[10] = (Data[2] >> 7) & 0x1F;  // Only 5 bits for bit 85
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        // XOR all data bits that this parity bit covers
        for (j = 0; j < 11; j++)
        {
            unsigned char masked = data_bytes[j] & HammingGenerator[i][j];
            // Count number of 1s (parity calculation)
            while (masked)
            {
                parity ^= (masked & 1);
                masked >>= 1;
...
int GNavBit::SetIonoUtc(PIONO_PARAM IonoParam, PUTC_PARAM UtcParam)
{
    ...
    // Note: For proper GLONASS implementation, we need GLONASS-specific parameters
    // that are not available in generic IONO_PARAM and UTC_PARAM structures
    
    unsigned int NA = 0;      // Day within four-year almanac period (bits 5-15)
    signed int tau_c = 0;     // System time scale correction (bits 16-47)
    unsigned int N4 = 0;      // Four-year period number (bits 49-53)
    signed int tau_GPS = 0;   // GLONASS-GPS time correction (bits 54-75)
    unsigned int ln5 = 0;     // String 5 validity flag (bit 76)
    
    // If UTC parameters are available, try to derive some values
    if (UtcParam && (UtcParam->flag & 0x3))
    {
        // Set validity flag if we have UTC data
        ln5 = 1;
        
        // Calculate N4 from week number (approximate)
        // GLONASS epoch: 1 Jan 1996 = GPS week 834
        // Each GLONASS 4-year period = 1461 days ≈ 208.7 weeks
        if (UtcParam->WN >= 834)
        {
            int weeks_since_glonass_epoch = UtcParam->WN - 834;
            N4 = weeks_since_glonass_epoch / 209;  // Approximate 4-year periods
            
            // Calculate day within current 4-year period
            int days_in_current_period = (weeks_since_glonass_epoch % 209) * 7;
            NA = days_in_current_period % 1461;
        }
        
        // tau_GPS approximation based on leap seconds
        // GPS-GLONASS time difference includes:
        // - 3 hour offset (10800 seconds)
        // - Leap second difference
        // This is a simplified approach - actual implementation needs proper parameters
        if (UtcParam->flag & 0x2)  // leap second available
        {
            // tau_GPS = (GPS_time - GLONASS_time) * 2^30 (scaled)
            // Approximate: GPS uses TAI-19s, GLONASS uses UTC+3h
            // tau_GPS ≈ (19 - TLS - 10800) seconds, scaled by 2^30
            double time_diff = 19.0 - UtcParam->TLS - 10800.0;
            tau_GPS = (signed int)(time_diff * (1L << 30));
        }
        
        // tau_c would need GLONASS-specific UTC correction
        // Not available in generic UTC_PARAM, so leaving at 0
    }
    
    // Fill string 5 for all 5 frames
    for (int frame = 0; frame < 5; frame++)
    {
        // String 5 structure per ICD GLONASS:
...
// ComposeStringEph and ComposeStringAlm remain the same as they only prepare the data
// The actual encoding and formatting is handled in GetFrameData
